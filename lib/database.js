...
export function getDatabase(databaseName) {
  ...
  const db = {
    ...
    async findMany(collection) {
      const { documents } = await req(collection, 'find');
  ...
  async function applyPendingMigrations(migrations) {
    ...
    const startIndex = lastMigration
      ? sortedMigrations.findIndex((m) => m.timestamp > lastMigration.timestamp)
      : 0;
    const pendingMigrations = sortedMigrations.slice(startIndex);
